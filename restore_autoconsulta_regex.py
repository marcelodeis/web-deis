...
if 'data-tab="autoconsulta"' not in content:
    content = re.sub(r'(\s*</div>\s*</header>)', '\n' + tab_btn_insert, content, count=1)
...
if 'id="tab-autoconsulta"' not in content:
    content = re.sub(r'(\s*</div>\s*</main>)', '\n' + tab_content_insert, content, count=1)
...
if 'autoconsulta.js' not in content:
    content = re.sub(r'(\s*<script src="data/comunas_osorno\.js"></script>\s*<script src="data/data_establecimientos\.js"></script>\s*<script src="bundle\.js\?v=.*?"></script>)', '\n' + scripts_insert, content, count=1)

with codecs.open(path, 'w', 'utf-8') as f:
    f.write(content)

print("Restored Autoconsulta to Covid_Web! Checked with regex.")
