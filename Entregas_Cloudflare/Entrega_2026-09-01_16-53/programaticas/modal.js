...
function openHelpModal(chartId) {
    ...
    function getDynamicGlossary(htmlContent) {
        ...
        if (text.includes('puntos porcentuales') || text.includes('(pp)') || text.includes(' pp ') || text.includes(' pp<')) {
            defs.push('<li style="margin-bottom: 4px;"><strong>Puntos Porcentuales (pp):</strong> Diferencia aritmética absoluta entre dos porcentajes. Útil para medir brechas exactas de cobertura.</li>');
        }
        if (text.includes('brecha inter-comunal') || text.includes('brecha inter-territorial') || text.includes('inequidad territorial')) {
            defs.push('<li style="margin-bottom: 4px;"><strong>Brecha Inter-territorial:</strong> Diferencia en pp entre el territorio con mayor y menor cobertura, evidenciando inequidad en salud y focalización de vulnerabilidad.</li>');
        }
        if (text.includes('base residencia') || text.includes('base ocurrencia') || text.includes('criterio territorial')) {
            defs.push('<li style="margin-bottom: 4px;"><strong>Base Residencia vs Ocurrencia:</strong> Residencia asigna la dosis según donde vive el paciente (para ver inmunidad territorial). Ocurrencia según donde se vacunó (para evaluar esfuerzo de la red).</li>');
        }
        ...
        <div style="margin-top: 20px; padding-top: 12px; border-top: 1px solid #e2e8f0; font-size: 0.75rem; color: #64748b; line-height: 1.4; text-align: left;">
            ...
            <div style="margin-top: 8px; font-style: italic; color: #94a3b8;">
                Referencias:<br>Organización Mundial de la Salud [OMS]. (2020). <em>Monitoreo de equidad en inmunización</em>. Ginebra, Suiza.
            </div>
        </div>`;
    }

    bodyHtml += getDynamicGlossary(bodyHtml);
