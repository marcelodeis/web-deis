 ...
 * â•‘   AUTOCONSULTA DE ESTADO DE VACUNACIÃ“N - COVID-19             â•‘
 * â•‘   MÃ³dulo de cruce de RUNs en navegador                         â•‘
 * â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
 *
 * Lee un Excel subido por el usuario, normaliza los RUNs,
 * cruza contra COVID_RUNS_INDEX (Set pre-generado),
 * y genera un Excel de salida con columna COVID-19 (SI/NO).
 *
 * Dependencias:
 *   - SheetJS (xlsx-js-style) â€” ya cargado en el proyecto
 *   - COVID_RUNS_INDEX.js â€” Set con RUNs vacunados
 */

// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
//  NORMALIZACIÃ“N DE RUN (port del algoritmo Python mÃ³dulo 11)
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

/**
 * Calcula el dÃ­gito verificador (DV) oficial de un RUN chileno.
 * Algoritmo mÃ³dulo 11 con multiplicadores 2-7 en ciclo.
 */
...
function normalizarRunSinDv(val) {
    ...
    // Caso 1: termina en 'k' â†’ definitivamente tiene DV
    if (s.endsWith('k')) {
        return s.slice(0, -1);
    }

    // Caso 2: todo numÃ©rico
    if (/^\d+$/.test(s)) {
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
//  DETECCIÃ“N AUTOMÃTICA DE COLUMNA RUN
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

/**
 * Busca en los headers del Excel la columna que contiene RUNs.
 * Retorna el Ã­ndice de la columna o -1 si no la encuentra.
 */
function detectarColumnaRun(headers) {
    const keywords = ['run', 'rut', 'rutificado', 'rÃºn', 'r.u.n', 'r.u.t'];
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
//  MÃ“DULO PRINCIPAL DE AUTOCONSULTA
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
const Autoconsulta = {
    ...
    /**
     * Inicializa los event listeners del mÃ³dulo.
     ...
     */
    processFile(file) {
        // Validar tamaÃ±o (mÃ¡ximo 150MB para evitar que el navegador se quede sin memoria)
        const maxSize = 150 * 1024 * 1024; // 150MB
        if (file.size > maxSize) {
            this.showError('El archivo es demasiado grande (mÃ¡ximo 150MB). Por favor divida su Excel para consultarlo. El error "out of memory" se produce al intentar cargar archivos gigantes en el navegador.');
            return;
        }
    ...
    _procesarWorkbook(arrayBuffer) {
        ...
        try {
            ...
            this.showError('Error al decodificar el Excel. El archivo podrÃ­a estar corrupto o usar un formato no soportado.');
            return;
        }
        ...
        if (!sheet || !sheet['!ref']) {
            this.showError('El archivo Excel estÃ¡ vacÃ­o.');
            return;
        }

        // Recalcular la Ãºltima fila real (ignorar filas vacÃ­as al final de la hoja)
        // En lugar de iterar un millÃ³n de filas, escaneamos solo las celdas que realmente existen
        ...
        // Verificar Ã­ndice
        if (typeof COVID_RUNS_INDEX === 'undefined') {
            this.showError('El Ã­ndice de vacunaciÃ³n no estÃ¡ disponible. Contacte al administrador del sistema.');
            return;
        }

        // Crear header de COVID-19
        const newColIdx = range.e.c + 1;
        const headerCellRef = XLSX.utils.encode_cell({r: range.s.r, c: newColIdx});
        sheet[headerCellRef] = { t: 's', v: 'COVID-19' };

        // Estilos para la cabecera (solo funcionarÃ¡n con js-style)
        ...
        const batchSize = 25000; // Chunk gigante ya que la operaciÃ³n es muy ligera
                ...
                for (; currentRow < end; currentRow++) {
                    ...
                    if (!runStr) {
                        ...
                        if (!runNorm || !/^\d+$/.test(runNorm)) {
                            ...
                            duplicadosCount++;
                            if (COVID_RUNS_INDEX.has(runNorm)) {
                                result = 'SI';
                            }
                        } else {
    ...
    _finalizarProcesamiento(workbook, detectedHeader, totalRows, siCount, noCount, vaciosCount, duplicadosCount, invalidosCount, statsComunas) {
        ...
        setTimeout(() => {
            this.updateProgress(100, 'Â¡Todo listo!');
            setTimeout(() => this.showResults(), 300);
        }, 100);
    },

    /**
     * Muestra el panel de resultados con estadÃ­sticas.
     */
    showResults() {
...
× ${stats.no} personas sin registro de vacuna Influenza 2026.">
    ...
    /**
     * Descarga el Excel procesado con la columna COVID-19.
     */
    downloadResult() {
        // Close dropdown
        const menu = document.getElementById('exportDropdownMenu');
        ...
        try {
            ...
            const outputName = `${baseName}_AUTOCONSULTA_COVID.xlsx`;
    ...
    /**
     * Reinicia el mÃ³dulo para procesar otro archivo.
     */
    reset() {
        this._state = {
            originalWorkbook: null,
