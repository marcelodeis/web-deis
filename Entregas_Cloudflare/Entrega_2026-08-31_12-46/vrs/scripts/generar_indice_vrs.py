...
â•‘   GENERADOR DE ÃNDICE DE RUNs - VRS RESIDENCIA 2026      â•‘
â•‘   Para mÃ³dulo Autoconsulta de Estado de VacunaciÃ³n              â•‘
â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

DESCRIPCIÃ“N:
    Lee el CSV de VRS_Residencia_2026.csv (~190 MB),
    aplica los 4 filtros obligatorios DEIS/MINSAL,
    normaliza los RUNs (sin DV, algoritmo mÃ³dulo 11),
    y genera un archivo JS ligero con un Set de RUNs Ãºnicos.
...
USO:
    python generar_indice_VRS.py

SALIDA:
    ../vrs_runs_index.js  (~1-2 MB)
"""
...
PROJECT_DIR = os.path.dirname(SCRIPT_DIR)  # VRS_Web/
...
OUTPUT_JS = os.path.join(PROJECT_DIR, "vrs_runs_index.js")

# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  FUNCIONES DE NORMALIZACIÃ“N DE RUN
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def main():
    print("=" * 65)
    print("  GENERADOR DE INDICE DE RUNs - VRS RESIDENCIA 2026")
    print("=" * 65)
    ...
    # --- PASO 2: Leer CSV y extraer RUNs vÃ¡lidos ---
    ...
    with open(CSV_PATH, "r", encoding=used_encoding) as f:
        ...
        for row in reader:
            ...
            # â”€â”€ Filtros obligatorios DEIS/MINSAL â”€â”€
    ...
    print(f"      RUNs unicos vÃ¡lidos: {len(runs_set):,}")
    ...
    with open(OUTPUT_JS, "w", encoding="utf-8") as f:
        ...
        f.write("// ÃNDICE DE RUNs VACUNADOS - VRS RESIDENCIA 2026\n")
        f.write("// Generado automÃ¡ticamente por generar_indice_VRS.py\n")
        f.write(f"// Total RUNs Ãºnicos: {len(runs_sorted):,}\n")
        f.write(f"// Fecha generaciÃ³n: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        ...
        f.write("const VRS_RUNS_INDEX = new Set([\n")

        # Escribir en bloques de 20 RUNs por lÃ­nea para legibilidad
        batch_size = 20
        for i in range(0, len(runs_sorted), batch_size):
            batch = runs_sorted[i:i + batch_size]
            line = ",".join(f'"{r}"' for r in batch)
            if i + batch_size < len(runs_sorted):
                f.write(f"  {line},\n")
            else:
                f.write(f"  {line}\n")
    ...
    print(f"{'=' * 65}")
    print(f"  RUNs Ãºnicos en el Ã­ndice : {len(runs_sorted):,}")
