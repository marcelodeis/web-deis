...
def process_year(year, ocurrencia_file, residencia_file, metas_file, output_js):
    ...
    ocu_grouped_mes = df_ocu.groupby(['COMUNA_OCURR', 'ESTABLECIMIENTO', 'CRITERIO_ELEGIBILIDAD', 'VAC_KEY', 'MES']).size().reset_index(name='count')
    ocu_grouped_se = df_ocu.groupby(['COMUNA_OCURR', 'ESTABLECIMIENTO', 'CRITERIO_ELEGIBILIDAD', 'VAC_KEY', 'SE']).size().reset_index(name='count')
    
    dict_ocu = {}
    for _, r in ocu_grouped_mes.iterrows():
        k = (r['COMUNA_OCURR'], r['ESTABLECIMIENTO'], r['CRITERIO_ELEGIBILIDAD'])
