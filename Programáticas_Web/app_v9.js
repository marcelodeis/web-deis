...
function switchYear(year) {
    ...
    if (reportDate) {
        if (isCobertura) {
            ...
            reportDate.innerText = year === '2025' ? `Fuente: DEIS-MINSAL, Fecha de corte: 31/12/2025 (Base Ocurrencia)` : `Fuente: DEIS-MINSAL, Fecha de corte: 05/07/2026 (Base Ocurrencia)`;
        }
    }
    ...
    if (fechaCorteSelect) {
        if (year === '2025') {
            ...
            fechaCorteSelect.innerHTML = `
                <option value="all">Actual (05/07/2026)</option>
    ...
    if (notaProduccion) {
        if (year === '2025') {
            ...
            notaProduccion.innerHTML = `
                <strong><i class="fas fa-info-circle"></i> Nota de Ocurrencia (Producción) - CORTE AL 05/07/2026:</strong> 
                Esta sección refleja la <b>producción bruta</b> (total de dosis administradas en el establecimiento), independientemente de la comuna de residencia del paciente. Los datos no corresponden al corte de marzo, sino a la fecha de extracción de la base de datos (05/07/2026).`;
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('reportDate').innerText = 
        `Fuente: DEIS-MINSAL, Fecha de corte: 05/07/2026 (Base Ocurrencia)`;
        ...
        btn.addEventListener('click', () => {
            ...
            if (tabId === 'cobertura') {
                ...
                if (reportDate) reportDate.innerText = currentYear === '2025' 
                    ? `Fuente: DEIS-MINSAL, Fecha de corte: 31/12/2025 (Base Ocurrencia)` 
                    : `Fuente: DEIS-MINSAL, Fecha de corte: 05/07/2026 (Base Ocurrencia)`;
                if (epiBadge) {
