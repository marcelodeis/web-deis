...
function switchYear(year) {
    ...
    if (reportDateEl && dashboardData.fecha_actualizacion) {
        reportDateEl.innerText = `Fuente: DEIS-MINSAL, Fecha de corte: ${dashboardData.fecha_actualizacion}`;
    }
...
function updateTableData() {
    ...
    const data_ocur = filter === 'all' ? tableDataObj.data_ocurrencia : tableDataObj.data_ocurrencia.filter(i => i.comuna === filter);
...
async function init() {
    try {
        ...
        const reportDateEl = document.getElementById('reportDate');
        if (reportDateEl) reportDateEl.innerText = `Fuente: DEIS-MINSAL, Fecha de corte: ${dashboardData.fecha_actualizacion}`;
    ...
    if (tableSearch) {
        tableSearch.addEventListener('keyup', function() {
            ...
            if (filterValue) {
                ...
                renderDoughnutChart(data_ocur, globalFilter);
                renderCriterioChart(data_ocur, globalFilter);
            } else {
                renderBarChart(data_ocur);
                renderDoughnutChart(data_ocur, globalFilter);
                const data_resi = globalFilter === 'all' ? dashboardData.data_residencia : dashboardData.data_residencia.filter(i => i.comuna === globalFilter);
...
function renderAll() {
    ...
    const data_resi = filter === 'all' ? dashboardData.data_residencia : dashboardData.data_residencia.filter(i => i.comuna === filter);
    
    // Operativo (Ocurrencia)
    const data_ocur = filter === 'all' ? dashboardData.data_ocurrencia : dashboardData.data_ocurrencia.filter(i => i.comuna === filter);
            ...
            const marker = L.marker(coords, { icon: customIcon }).bindPopup(`
                <div style="text-align:center;">
                    ...
                    <div style="margin-top:8px; padding:4px; background:${col}20; color:${col}; border-radius:4px; font-weight:bold;">
                        Riesgo Comunal: ${comunaPerc.toFixed(1)}%
                    </div>
                </div>
            `);
...
function renderTimeSeriesChart(data, filter) {
    ...
    const avanceData2025 = typeof DASHBOARD_DATA_OFFLINE_2025 !== 'undefined' ? (DASHBOARD_DATA_OFFLINE_2025.avance_semanal || {}) : {};
    ...
    if (seKeys.length === 0) {
        ...
        seKeys.forEach(se => {
            ...
            avanceAcumulado.push(parseFloat(pct.toFixed(1)));
        });
    }

    // Forecasting (Proyección de Término)
    let projectedLabels = [...labels];
    let projectedAvance = new Array(avanceAcumulado.length).fill(null);
    projectedAvance[projectedAvance.length - 1] = avanceAcumulado[avanceAcumulado.length - 1];
    
    if (avanceAcumulado.length >= 3 && seKeys.length >= 3) {
        const valNow = avanceAcumulado[avanceAcumulado.length - 1];
        const val3WeeksAgo = avanceAcumulado[avanceAcumulado.length - 4] || avanceAcumulado[0];
        const span = Math.min(3, avanceAcumulado.length - 1);
        const slope = span > 0 ? (valNow - val3WeeksAgo) / span : 0;
        
        if (slope > 0 && valNow < 80) {
            let currentProj = valNow;
            let seProj = seKeys[seKeys.length - 1];
            while(currentProj < 80 && projectedLabels.length < 52) {
                currentProj += slope;
                seProj++;
                projectedLabels.push('SE ' + seProj);
                projectedAvance.push(parseFloat(currentProj.toFixed(1)));
                if (currentProj >= 80) break;
            }
        }
    }
    
    // Ajustar arrays a projectedLabels
    const finalLabels = projectedLabels;
    const finalAvance = [...avanceAcumulado, ...new Array(finalLabels.length - labels.length).fill(null)];
    const finalDosis = [...dosisSemanales, ...new Array(finalLabels.length - labels.length).fill(null)];
    ...
    const maxDosis = Math.max(...dosisSemanales, 10);
