...
function getOcurrenciaData(comuna) {
    ...
    let filtered = DATA.data_ocurrencia;
    if (comuna !== 'all') {
        filtered = filtered.filter(d => d.comuna.toUpperCase() === comuna.toUpperCase());
    }
    
    const tipoFilter = document.getElementById('globalTipoFilter')?.value || 'all';
    if (tipoFilter !== 'all') {
        filtered = filtered.filter(d => {
            const estUpper = (d.establecimiento || '').toUpperCase();
            const isPrivado = estUpper.includes('ALEMANA') || 
                              estUpper.includes('COCHRANE') || 
                              estUpper.includes('MUTUAL') || 
                              estUpper.includes('VAXPLUS');
...
function getPoblacionObjetivo(comuna) {
    ...
    const COMUNA_A_CODIGO = {
        'OSORNO': ['10301.0', '10301'],
        'PUERTO OCTAY': ['10302.0', '10302'],
        'PURRANQUE': ['10303.0', '10303'],
        'PUYEHUE': ['10304.0', '10304'],
        'RÍO NEGRO': ['10305.0', '10305'],
        'SAN JUAN DE LA COSTA': ['10306.0', '10306'],
        'SAN PABLO': ['10307.0', '10307']
    };
    
    if (comuna === 'all') {
        let total = 0;
        // Sum only the communes of Osorno Province
        Object.values(COMUNA_A_CODIGO).forEach(codes => {
            for (let code of codes) {
                if (DATA.metas[code]) {
                    total += DATA.metas[code].Poblacion_Objetivo || 0;
                    break; // stop checking codes for this commune once we found it
                }
            }
        });
        return total;
    } else {
        const codes = COMUNA_A_CODIGO[comuna.toUpperCase()];
        if (!codes) return 0;
        
        for (let code of codes) {
            if (DATA.metas[code]) return DATA.metas[code].Poblacion_Objetivo || 0;
        }
        return 0;
    }
}
    ...
    return {
            ...
            <div style="color:var(--text-primary,#334155);font-size:0.92rem;line-height:1.6;text-align:justify;">
                ...
                <div style="margin-top:16px;">
                    ...
                    <ul style="padding-left:20px;margin:8px 0 0 0;">
                        ...
                        <li style="margin-bottom:8px;"><strong>Estabilización de Demanda:</strong> Un <em>plateau</em> precoz (*Plateau: Fase de estabilización o meseta en la curva donde no hay crecimiento) exige barridos territoriales de rescate en grupos subrepresentados.</li>
                        <li>La solidez descansa en la exclusión de EPROs (*EPROs: Errores Programáticos, es decir, registros de vacunación con desviaciones del protocolo), ilustrando estrictamente la inmunidad comprobada en ${locName}.</li>
                    </ul>
                </div>
            </div>`
        },
        topEstablecimientos: {
            ...
            <div style="color:var(--text-primary,#334155);font-size:0.92rem;line-height:1.6;text-align:justify;">
                ...
                <div style="margin-top:16px;">
                    ...
                    <ul style="padding-left:20px;margin:8px 0 0 0;">
                        <li style="margin-bottom:8px;">Detecta precozmente la sobrecarga que precipita fallos en bioseguridad, ruptura de la red de frío o EPRO (*EPRO: Error Programático, fallo en los procedimientos estándar) por fatiga en nodos de alta presión.</li>
                        <li>Facilita reasignación dinámica de RRHH (*RRHH: Recursos Humanos) desde nodos infrautilizados hacia puntos calientes de vacunación.</li>
                    </ul>
                </div>
            </div>`
        },
        tipoCentro: {
            ...
            <div style="color:var(--text-primary,#334155);font-size:0.92rem;line-height:1.6;text-align:justify;">
                ...
                <div style="margin-top:16px;">
                    <strong><i class="fa-solid fa-lightbulb" style="margin-right:6px;color:#f59e0b;"></i>Integración Territorial RISS (*Redes Integradas de Servicios de Salud)</strong>
                    <ul style="padding-left:20px;margin:8px 0 0 0;">
                        ...
                        <li><strong>Contención del Riesgo Clínico Crítico:</strong> Las vacunaciones hospitalarias operan como el último cortafuegos antes del alta de pacientes inmunodeprimidos — previniendo reingresos a UPC (*UPC: Unidad de Paciente Crítico, incluye UCI y UTI) en la red ${currentYear}.</li>
                    </ul>
                </div>
            </div>`
        }
...
function openHelpModal(chartId, btnElement) {
    ...
    if (card) {
        ...
        data.body += getDynamicGlossary(data.body);
