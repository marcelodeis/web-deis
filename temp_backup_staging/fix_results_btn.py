...
for f in files:
    ...
    # We want to replace the `resultsArea.innerHTML = \`` block injected earlier with one that includes the download button.
    # The previous injection ended with:
    #             ${statsHTML}
    #         `;
    ...
    if "autoconsultaBtnDownload" not in content.split("resultsArea.innerHTML")[1]:
        content = content.replace("            ${statsHTML}\n        `;\n        resultsArea.style.display = 'block';", replacement)
        with open(path, 'w', encoding='utf-8') as file:
            file.write(content)
        print(f"Added download button to {f}")
    else:
        print(f"Download button already in {f}")
