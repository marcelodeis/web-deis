...
def fix_generate_pdf(content, vaccine_name):
    ...
    new_generate = '''    _generatePDF() {
        ...
        // VITAL FIX: Do NOT use left: -9999px. html2canvas will render a blank page if it's off screen bounds.
        // Instead, use absolute position, z-index -1, top 0, left 0, and background white.
        pdfContainer.style.cssText = 'position: absolute; top: 0; left: 0; z-index: -9999; width: 210mm; background-color: #ffffff; font-family: Arial, Helvetica, sans-serif; color: #1e293b; font-size: 11px; line-height: 1.6; padding: 20px;';
        ...
        // Very important: append to body before capturing, but hidden behind everything
        document.body.appendChild(pdfContainer);
        
        // Wait a tiny bit for the DOM to fully render the element before capturing
        setTimeout(() => {
            const opt = {
                margin: [5, 5, 5, 5],
...
for vaccine, path in files.items():
    ...
    # Replace _generatePDF and fix accents
    content = fix_generate_pdf(content, vaccine)
    
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
        
    print(f"Fixed {vaccine}")
