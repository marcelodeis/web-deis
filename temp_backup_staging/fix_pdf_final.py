...
def fix_generate_pdf_final(content, vaccine_name):
    ...
    new_generate = '''    _generatePDF() {
        ...
        // We explicitly set a fixed width of 800px so it perfectly matches A4 proportions.
        const htmlContent = `
            <div style="width: 800px; padding: 40px; box-sizing: border-box; background-color: #ffffff; font-family: Arial, Helvetica, sans-serif; color: #1e293b; font-size: 11px; line-height: 1.5;">
        ...
        const opt = {
            margin: [0, 0, 0, 0], // Margin removed, container padding handles it perfectly
        ...
        // Use pure HTML string - NO DOM appending, NO glitches!
...
for vaccine, path in files.items():
    ...
    content = fix_generate_pdf_final(content, vaccine)
    
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
        
    print(f"Fixed {vaccine} with FINAL flawless solution!")
